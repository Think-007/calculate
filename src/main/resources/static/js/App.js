...
/*过滤器--国际化*/
/*App.filter("T",function($translate) {
...
/*js国际化调用*/
App.factory('translateTip', function($translate) {
    var translateTip = {};
    translateTip.context=function(key,value) {
        if(key){
            return $translate.instant(key);
        }
        return key;
...
App.directive('btnPermission', ["auth",function (auth) {
    return {
        ...
        link: function(scope, element, attr) {
        	 ...
        	 //console.log("========================");
        	 //console.log(attr.btnPermission);
...
function getLanguage()
{
	try{
		...
		return (olang=="zh-CN")?"zh-CN":"en-US";
	}catch(e){
...
function bootstrapSelectLangInit(lang,jsFilePath) {
	...
	//语言环境
	if(lowerLanguage.indexOf("zh_CNgetErrorCodeFile")>=0){
...
//存储session,服务器端可从session中读取
setServerLang();
function setServerLang(){
	var lang=window.localStorage.lang || 'zh-CN';
	var langHeader=lang.split("-")[0];
	var obj={
		"locale":langHeader	
	}
	//调用接口
	setLocale(obj,function(data){
		console.log(data);
	});  
};