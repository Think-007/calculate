...
/*定义module*/
App.controller('userManageController', function($scope, $ocLazyLoad, translateTip){
	var pageNum=1;//页码
    ...
    function inputLanguage(){
    	...
    	if(lang=="zh-CN"){
    		 $scope.userInfoList={"inUse":"使用中","invalid":"已失效","approve":"通过","noReviewed":"未审核","reject":"不通过"} 
    	}else{
    		 $scope.userInfoList={"inUse":"In Use","invalid":"Invalid","approve":"Approve","noReviewed":"Not Reviewed","reject":"Reject"}
    	}
    }
...
App.controller("userSearch",['$scope',function($scope){
	...
	$scope.generateRoleOptions = function(roleList){
		...
		$("#rolelist").html('');
		var emptyOption= "<option value=''>All</option>";
